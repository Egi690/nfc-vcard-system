...
// Sync admin user with current env variables on every startup
const syncAdminUser = () => {
  ...
  if (!existing) {
    ...
    // Admin exists — update email + password to match current env
    db.prepare('UPDATE users SET email = ?, password = ? WHERE id = 1').run(adminEmail, hashedPassword);
...
app.post('/api/auth/login', (req, res) => {
  ...
  console.log('[LOGIN] user found in DB:', user ? user.email : 'NOT FOUND');

  if (!user) {
    console.log('[LOGIN] FAILED — no user with that email in DB');
