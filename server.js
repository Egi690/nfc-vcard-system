...
app.get('/api/summary', authenticateToken, (req, res) => {
  const totalCards = db.prepare('SELECT COUNT(*) as count FROM vcards WHERE user_id = ?').get(req.user.id).count;
  const activeCards = db.prepare('SELECT COUNT(*) as count FROM vcards WHERE user_id = ? AND is_active = 1').get(req.user.id).count;
  const totalScans = db.prepare('SELECT SUM(scan_count) as total FROM vcards WHERE user_id = ?').get(req.user.id).total || 0;
